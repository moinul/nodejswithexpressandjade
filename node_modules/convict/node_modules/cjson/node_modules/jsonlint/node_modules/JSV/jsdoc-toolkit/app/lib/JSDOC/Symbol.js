...
JSDOC.Symbol.prototype.setTags = function() {
	...
	*/
	...
	if (params.length) {
		// users-defined params overwrite those with same name defined by the parser
		var thisParams = this.params;

		if (thisParams.length == 0) { // none exist yet, so just bung all these users-defined params straight in
			this.params = params;
		}
		else { // need to overlay these users-defined params on to existing parser-defined params
	...
	/*t:
		...
		is(sym.params.length, 1, "users defined param is found.");
		is(sym.params[0].type, "array", "users defined param type is found.");
		is(sym.params[0].name, "pages", "users defined param name is found.");
		
		sym = new JSDOC.Symbol("foo", [{type: "array", name: "pages"}], "FUNCTION", new JSDOC.DocComment("/**Description.\n@param {string} uid*"+"/"));
		is(sym.params.length, 1, "users defined param overwrites parser defined param.");
		is(sym.params[0].type, "string", "users defined param type overwrites parser defined param type.");
		is(sym.params[0].name, "uid", "users defined param name overwrites parser defined param name.");
	
		sym = new JSDOC.Symbol("foo", [{type: "array", name: "pages"}, {type: "number", name: "count"}], "FUNCTION", new JSDOC.DocComment("/**Description.\n@param {string} uid*"+"/"));
		is(sym.params.length, 2, "users defined params  overlay parser defined params.");
		is(sym.params[1].type, "number", "users defined param type overlays parser defined param type.");
		is(sym.params[1].name, "count", "users defined param name overlays parser defined param name.");

		sym = new JSDOC.Symbol("foo", [], "FUNCTION", new JSDOC.DocComment("/**Description.\n@param {array} pages The pages description.*"+"/"));
		is(sym.params.length, 1, "users defined param with description is found.");
		is(sym.params[0].desc, "The pages description.", "users defined param description is found.");
	*/
